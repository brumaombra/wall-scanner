...
int PS2MouseHandler::initialise() {
    int counter = 0;
    int return_value = 0;
    do { // Poll mouse to get a connection
...
void PS2MouseHandler::write(int data) {
    ...
    for (i = 0; i < 8; i++) { // clock is low, and we are clear to send data
		...
		start_time = millis();
        while (!digitalRead(_clock_pin)) { // Loop with timeout
            if (millis() - start_time > MAX_CLOCK_TIMEOUT) return;
        }
		start_time = millis();
        while (digitalRead(_clock_pin)) { // Loop with timeout
			if (millis() - start_time > MAX_CLOCK_TIMEOUT) return;
	...
	// wait for mouse to take over data line
	start_time = millis();
	while (digitalRead(_data_pin)) { // Loop with timeout
		if (millis() - start_time > MAX_CLOCK_TIMEOUT) return;
	}

	// wait for mouse to take over clock
	start_time = millis();
	while (digitalRead(_clock_pin)) { // Loop with timeout
		if (millis() - start_time > MAX_CLOCK_TIMEOUT) return;
	}

	// wait for mouse to release clock and data
	start_time = millis();
	while ((!digitalRead(_clock_pin)) && (!digitalRead(_data_pin))) { // Loop with timeout
		if (millis() - start_time > MAX_CLOCK_TIMEOUT) return;
	}
}
...
uint8_t PS2MouseHandler::read_byte() {
	...
	unsigned long start_time = millis();
	while (digitalRead(_clock_pin)) { // Loop with timeout
		if (millis() - start_time > MAX_CLOCK_TIMEOUT) return 0;
	}
	start_time = millis();
	while (!digitalRead(_clock_pin)) { // Loop with timeout
		if (millis() - start_time > MAX_CLOCK_TIMEOUT) return 0;
